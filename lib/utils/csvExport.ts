...
export class CSVExporter {
  ...
  static async exportOrders(
    ...
    try {
      ...
      if (filters?.category) {
        filenameParts.push(`category-${filters.category.toLowerCase().replace(/\s+/g, '-')}`);
      }
      
      if (filters?.source) {
        filenameParts.push(`source-${filters.source.toLowerCase().replace(/\s+/g, '-')}`);
      }
      
      if (filters?.geo) {
        filenameParts.push(`location-${filters.geo.toLowerCase().replace(/\s+/g, '-')}`);
      }
