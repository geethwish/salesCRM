...
import { Order, OrderQuery, OrderListResponse, ApiResponse } from '@/lib/types/order';
    ...
    if (error.response) {
      ...
      switch (status) {
        ...
        case 500:
          ...
          break;
        default:
          const errorMessage = (data as ApiResponse)?.error?.message || 'An unexpected error occurred.';
...
export const ordersApi = {
  ...
  getOrders: async (query: Partial<OrderQuery> = {}): Promise<AxiosResponse<ApiResponse<OrderListResponse>>> => {
  ...
  // Create new order
  createOrder: async (orderData: Omit<Order, 'id' | 'createdAt' | 'updatedAt'>): Promise<AxiosResponse<ApiResponse<Order>>> => {
    return apiClient.post('/api/orders', orderData);
  },

  // Update existing order
  updateOrder: async (id: string, orderData: Partial<Order>): Promise<AxiosResponse<ApiResponse<Order>>> => {
    return apiClient.put(`/api/orders/${id}`, orderData);
  },

  // Delete order
  deleteOrder: async (id: string): Promise<AxiosResponse<ApiResponse<void>>> => {
