...
import { LoginRequest, RegisterRequest, UpdateProfile, ChangePassword, UseAuthReturn } from '@/lib/types/auth';
...
export function useAuth(): UseAuthReturn {
  ...
  const login = useCallback(async (credentials: LoginRequest): Promise<void> => {
    ...
    if (loginUser.fulfilled.match(result)) {
      ...
      // Error is already handled in the thunk (toast notification)
  ...
  const updateProfile = useCallback(async (data: UpdateProfile): Promise<void> => {
  ...
  const changePassword = useCallback(async (data: ChangePassword): Promise<void> => {
