    ...
    if (response.statusCode === 200) {
      ...
      const checks = [
        { name: "HTML structure", test: html.includes("<!DOCTYPE html>") },
        {
          ...
          test: html.includes('id="swagger-ui"'),
        },
        {
          name: "Swagger UI bundle script",
          test: html.includes("swagger-ui-bundle.js"),
        },
        {
          name: "Swagger UI preset script",
          test: html.includes("swagger-ui-standalone-preset.js"),
        },
        { name: "Swagger UI CSS", test: html.includes("swagger-ui.css") },
        {
          name: "OpenAPI JSON URL",
          test: html.includes("/api/docs/openapi.json"),
        },
        { name: "Error handling", test: html.includes("initializeSwaggerUI") },
        {
          name: "Loading indicator",
          test: html.includes("Loading API Documentation"),
        },
      ];
      ...
      if (csp) {
        ...
        console.log(
          `   - Allows unpkg.com scripts: ${csp.includes("https://unpkg.com")}`
        );
        console.log(
          `   - Allows unpkg.com styles: ${csp.includes("https://unpkg.com")}`
        );
        console.log(
          `   - Allows unsafe-inline: ${csp.includes("'unsafe-inline'")}`
        );
        console.log(
          `   - Allows unsafe-eval: ${csp.includes("'unsafe-eval'")}`
        );
      }
